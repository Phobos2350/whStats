...
class GenerateStats {
  ...
  public function listenForChanges() {
    ...
    //$this->rethinkQueries->queueTask('genPilotStats', '', '', $data);
    $this->rethinkQueries->queueTask('getPilotStats', 'pilotStats', $key, $data);
    foreach($feed as $change) {
      ...
      if($thisKillID != $currentKillID) {
        ...
        $this->rethinkQueries->queueTask('getEntityStats', 'entityStats', $key, $data);
        $key = md5(strtoupper('pilotStats_month_'.$killYear.'_'.$killMonth));
        $this->rethinkQueries->queueTask('addPilotStatsMonth', '', '', $data);
        $this->rethinkQueries->queueTask('getPilotStats', 'pilotStats', $key, $data);
        ...
        if($killTime >= strtotime('-1 hour') || strtotime("-2 minutes") >= $lastCheck) {
          ...
          $this->rethinkQueries->queueTask('genEntityStats', '', '', $data);
          $this->rethinkQueries->queueTask('getEntityStats', 'entityStats', $key, $data);
          $key = md5(strtoupper('pilotStats_hour_0_0'));
          $this->rethinkQueries->queueTask('genPilotStats', '', '', $data);
          $this->rethinkQueries->queueTask('getPilotStats', 'pilotStats', $key, $data);
        }
        if($killTime >= strtotime('-1 day') && $killTime < strtotime('-1 hour') || strtotime("-5 minutes") >= $lastDayCheck) {
          ...
          $this->rethinkQueries->queueTask('genEntityStats', '', '', $data);
          $this->rethinkQueries->queueTask('getEntityStats', 'entityStats', $key, $data);
          $key = md5(strtoupper('pilotStats_day_0_0'));
          //$this->rethinkQueries->queueTask('genPilotStats', '', '', $data);
          $this->rethinkQueries->queueTask('getPilotStats', 'pilotStats', $key, $data);
        }
        if($killTime >= strtotime('-1 week') && $killTime < strtotime('-1 day') || strtotime("-30 minutes") >= $lastWeekCheck) {
          ...
          $this->rethinkQueries->queueTask('genEntityStats', '', '', $data);
          $this->rethinkQueries->queueTask('getEntityStats', 'entityStats', $key, $data);
          $key = md5(strtoupper('pilotStats_week_0_0'));
          //$this->rethinkQueries->queueTask('genPilotStats', '', '', $data);
          $this->rethinkQueries->queueTask('getPilotStats', 'pilotStats', $key, $data);
        }
  ...
  public function genPilotStats($period, $year, $month) {
    ...
    if($period != 'month') {
      ...
      while($continue) {
        ...
        ->merge(function($row) {
          return array(
            'characterID' => $row('group'),
            'characterName' => $row('reduction')('characterName')->distinct(),
            'corporationID' => $row('reduction')('corporationID')->distinct(),
            'corporationName' => $row('reduction')('corporationName')->distinct(),
            'allianceID' => $row('reduction')('allianceID')->distinct(),
            'allianceName' => $row('reduction')('allianceName')->distinct(),
            'totalKills' => $row('reduction')('killID')->distinct()->count(),
            'totalISK' => $row('reduction')('value')->distinct()->sum()
          );
        })
        ->map(function($each) {
          return array(
            'characterID' => $each('characterID'),
            'characterName' => $each('characterName'),
            'corporationID' => $each('corporationID'),
            'corporationName' => $each('corporationName'),
            'allianceID' => $each('allianceID'),
            'allianceName' => $each('allianceName'),
            'totalISK' => $each('totalISK'),
            'totalKills' => $each('totalKills'),
            'killsArray' => $each('reduction')
          );
    ...
    foreach($combinedResults as $entity) {
      ...
      if($entityID != 0) {
        ...
        if(!isset($toEncode['stats']['ALL'][$entityIDStr])) {
          $toEncode['stats']['ALL'][$entityIDStr] = array();
          $toEncode['stats']['ALL'][$entityIDStr]['entityID'] = $entity['characterID'];
          $toEncode['stats']['ALL'][$entityIDStr]['entityName'] = $entity['characterName'];
          $toEncode['stats']['ALL'][$entityIDStr]['corporationID'] = $entity['corporationID'];
          $toEncode['stats']['ALL'][$entityIDStr]['corporationName'] = $entity['corporationName'];
          $toEncode['stats']['ALL'][$entityIDStr]['allianceID'] = $entity['allianceID'];
          $toEncode['stats']['ALL'][$entityIDStr]['allianceName'] = $entity['allianceName'];
          $toEncode['stats']['ALL'][$entityIDStr]['totalKills'] = 0;
          $toEncode['stats']['ALL'][$entityIDStr]['totalISK'] = 0;
          $toEncode['stats']['ALL'][$entityIDStr]['shipsUsed'] = array();
          for($i = 1; $i < 10; $i++) {
        ...
        if(!isset($toEncode['stats']['US'][$entityIDStr])) {
          $toEncode['stats']['US'][$entityIDStr] = array();
          $toEncode['stats']['US'][$entityIDStr]['entityID'] = $entity['characterID'];
          $toEncode['stats']['US'][$entityIDStr]['entityName'] = $entity['characterName'];
          $toEncode['stats']['US'][$entityIDStr]['corporationID'] = $entity['corporationID'];
          $toEncode['stats']['US'][$entityIDStr]['corporationName'] = $entity['corporationName'];
          $toEncode['stats']['US'][$entityIDStr]['allianceID'] = $entity['allianceID'];
          $toEncode['stats']['US'][$entityIDStr]['allianceName'] = $entity['allianceName'];
          $toEncode['stats']['US'][$entityIDStr]['totalKills'] = 0;
          $toEncode['stats']['US'][$entityIDStr]['totalISK'] = 0;
          $toEncode['stats']['US'][$entityIDStr]['shipsUsed'] = array();
          for($i = 1; $i < 10; $i++) {
        ...
        if(!isset($toEncode['stats']['AU'][$entityIDStr])) {
          $toEncode['stats']['AU'][$entityIDStr] = array();
          $toEncode['stats']['AU'][$entityIDStr]['entityID'] = $entity['characterID'];
          $toEncode['stats']['AU'][$entityIDStr]['entityName'] = $entity['characterName'];
          $toEncode['stats']['AU'][$entityIDStr]['corporationID'] = $entity['corporationID'];
          $toEncode['stats']['AU'][$entityIDStr]['corporationName'] = $entity['corporationName'];
          $toEncode['stats']['AU'][$entityIDStr]['allianceID'] = $entity['allianceID'];
          $toEncode['stats']['AU'][$entityIDStr]['allianceName'] = $entity['allianceName'];
          $toEncode['stats']['AU'][$entityIDStr]['totalKills'] = 0;
          $toEncode['stats']['AU'][$entityIDStr]['totalISK'] = 0;
          $toEncode['stats']['AU'][$entityIDStr]['shipsUsed'] = array();
          for($i = 1; $i < 10; $i++) {
        ...
        if(!isset($toEncode['stats']['EU'][$entityIDStr])) {
          $toEncode['stats']['EU'][$entityIDStr] = array();
          $toEncode['stats']['EU'][$entityIDStr]['entityID'] = $entity['characterID'];
          $toEncode['stats']['EU'][$entityIDStr]['entityName'] = $entity['characterName'];
          $toEncode['stats']['EU'][$entityIDStr]['corporationID'] = $entity['corporationID'];
          $toEncode['stats']['EU'][$entityIDStr]['corporationName'] = $entity['corporationName'];
          $toEncode['stats']['EU'][$entityIDStr]['allianceID'] = $entity['allianceID'];
          $toEncode['stats']['EU'][$entityIDStr]['allianceName'] = $entity['allianceName'];
          $toEncode['stats']['EU'][$entityIDStr]['totalKills'] = 0;
          $toEncode['stats']['EU'][$entityIDStr]['totalISK'] = 0;
          $toEncode['stats']['EU'][$entityIDStr]['shipsUsed'] = array();
          for($i = 1; $i < 10; $i++) {
      ...
      $record = array('key' => $key, 'stats' => $toEncode['stats']);
      $documentExists = r\table('generatedPilotStats')->get($key)->run($conn);
      if($documentExists != null) {
        $result = r\table('generatedPilotStats')->get($key)->replace($record)->run($conn);
      } else {
        $result = r\table('generatedPilotStats')->insert($record)->run($conn);
      }
    }
    $conn->close();
  }

  public function addPilotStatsMonth($kill, $period, $year, $month) {
    $key = md5(strtoupper('pilotStats_'.$period.'_'.$year.'_'.$month));
    $entityKillSeen = array();
    $toEncode = array();
    $conn = r\connect('localhost', 28015, 'stats');
    ...
    $toEncode = r\table('generatedPilotStats')->get($key)->run($conn);
    foreach($kill['attackers'] as $attacker) {
      $entityID = $attacker['characterID'];
      $entityName = $attacker['characterName'];
      $entityCorpID = $attacker['corporationID'];
      $entityCorpName = $attacker['corporationName'];
      $entityAllianceID = $attacker['allianceID'];
      $entityAllianceName = $attacker['allianceName'];
      if($entityID != 0) {
        if(!in_array($entityID, $entityKillSeen)) {
          $arrayKey = array_search($entityID, array_column($toEncode['stats']['ALL'], 'entityID'));
          if(!isset($toEncode['stats']['ALL'][$arrayKey])) {
            $toEncode['stats']['ALL'][$arrayKey] = array();
            $toEncode['stats']['ALL'][$arrayKey]['characterID'] = $entityID;
            $toEncode['stats']['ALL'][$arrayKey]['characterName'] = $entityName;
            $toEncode['stats']['ALL'][$arrayKey]['corporationID'] = $entityCorpID;
            $toEncode['stats']['ALL'][$arrayKey]['corporationName'] = $entityCorpName;
            $toEncode['stats']['ALL'][$arrayKey]['allianceID'] = $entityAllianceID;
            $toEncode['stats']['ALL'][$arrayKey]['allianceName'] = $entityAllianceName;
            $toEncode['stats']['ALL'][$arrayKey]['totalKills'] = 0;
            $toEncode['stats']['ALL'][$arrayKey]['totalISK'] = 0;
            $toEncode['stats']['ALL'][$arrayKey]['shipsUsed'] = array();
            for($i = 1; $i < 10; $i++) {
          ...
          if(!isset($toEncode['stats'][$killTimezone][$arrayKey])) {
            $toEncode['stats'][$killTimezone][$arrayKey] = array();
            $toEncode['stats'][$killTimezone][$arrayKey]['characterID'] = $entityID;
            $toEncode['stats'][$killTimezone][$arrayKey]['characterName'] = $entityName;
            $toEncode['stats'][$killTimezone][$arrayKey]['corporationID'] = $entityCorpID;
            $toEncode['stats'][$killTimezone][$arrayKey]['corporationName'] = $entityCorpName;
            $toEncode['stats'][$killTimezone][$arrayKey]['allianceID'] = $entityAllianceID;
            $toEncode['stats'][$killTimezone][$arrayKey]['allianceName'] = $entityAllianceName;
            $toEncode['stats'][$killTimezone][$arrayKey]['totalKills'] = 0;
            $toEncode['stats'][$killTimezone][$arrayKey]['totalISK'] = 0;
            $toEncode['stats'][$killTimezone][$arrayKey]['shipsUsed'] = array();
            for($i = 1; $i < 10; $i++) {
